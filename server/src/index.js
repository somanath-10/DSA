...
// Setup dynamic CORS configuration support
...
app.use(cors({
  origin: function (origin, callback) {
    // If no origin (like mobile apps, curl, postman, direct server to server), allow it
    if (!origin) return callback(null, true);
    
    // Check if the current origin matches any of the allowed origins or wildcard
    if (allowedOrigins.includes('*') || allowedOrigins.includes(origin)) {
      return callback(null, true);
    }
    
    // Fallback block allowing localhost defaults if it fails
